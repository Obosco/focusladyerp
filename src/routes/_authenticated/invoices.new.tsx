...
function InvoiceEditor() {
  ...
  return <div className="grid gap-6 xl:grid-cols-[36rem_1fr]">
    <div className="space-y-4 print:hidden">
      ...
      <Card><CardHeader className="flex-row items-center justify-between"><CardTitle className="text-sm">Product Rows</CardTitle><Button variant="outline" size="sm" className="rounded-none" onClick={() => setItems((current) => [...current, emptyItem()])}><Plus className="mr-1 h-4 w-4" />Add row</Button></CardHeader><CardContent className="space-y-2">{items.map((item, index) => <div key={index} className="grid grid-cols-12 gap-2 border-b border-black/10 pb-2"><Field label="Description" value={item.product} setValue={(value) => chooseProduct(index, value)} list="products" className="col-span-5" />{mode === "gst" && <Field label="HSN" value={item.hsn} setValue={(value) => setItem(index, { hsn: value })} className="col-span-2" />}<Field label="Qty" value={String(item.qty)} setValue={(value) => setItem(index, { qty: Number(value) })} type="number" className="col-span-2" /><Field label="Rate" value={String(item.rate)} setValue={(value) => setItem(index, { rate: Number(value) })} type="number" className="col-span-2" /><Button variant="ghost" size="icon" className="mt-4 h-8 w-8" disabled={items.length === 1} onClick={() => setItems((current) => current.filter((_, itemIndex) => itemIndex !== index))}><Trash2 className="h-4 w-4" /></Button>{mode === "gst" && <Field label="GST %" value={String(item.gstPercent)} setValue={(value) => setItem(index, { gstPercent: Number(value) })} type="number" className="col-span-2" />}<div className="col-span-5 self-end text-right text-xs">Taxable {money(itemValues(item, mode).taxable)} | Total {money(itemValues(item, mode).total)}</div></div>)}<datalist id="products">{products.map((row, index) => <option key={index} value={row[1]} />)}</datalist><datalist id="customers">{customers.map((row, index) => <option key={`${index}-name`} value={row[1]} /><option key={`${index}-phone`} value={row[2]} />)}</datalist></CardContent></Card>
